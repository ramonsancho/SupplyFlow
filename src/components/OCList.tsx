...
export default function OCList() {
  ...
  const handleApprove = async (po: PurchaseOrder) => {
    ...
    const isSuperAdmin = currentUserProfile.role === 'Administrador' || isBootstrapAdmin(currentUserProfile.email) || isBootstrapAdmin(auth.currentUser?.email);
            ...
            >
              ...
              <div className="flex flex-col gap-2 min-w-[280px] self-center">
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  {!isRequisitioner(currentUserProfile?.role) && hasApprovalPermission(currentUserProfile) && (oc.status === 'approved' || oc.status === 'sent' || oc.status === 'received') && (
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedPO(oc);
